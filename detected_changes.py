...
def detect_column_changes(old_path, new_path):
    ...
    renamed = {
        ...
        if (match := difflib.get_close_matches(old_col, new_cols, n=1, cutoff=0.85)) and match[0] not in old_cols
    }
...
def detect_table_changes(old_tables, new_tables):
    ...
    for old_name in old_set - new_set:
        match = difflib.get_close_matches(old_name, added, n=1, cutoff=0.85)
        if match:
